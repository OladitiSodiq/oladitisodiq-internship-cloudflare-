 ...
 */
async function handleRequest(request) { let urls = "";
...
class ElementHandler {
  ...
  element(element) {
    if (element.tagName === 'title') {
      element.setInnerContent('Oladiti Sodiq')
    }
    ...
    if (element.tagName === 'p' && element.getAttribute('id') === 'description') {
      if (this.elementHandler === 'variant-1') {
        element.setInnerContent("View my portfolio.")
      } else {
    ...
    if (element.tagName === 'a' && element.getAttribute('id') === 'url') {
      ...
      if (this.elementHandler === 'variant-1') {
        element.setAttribute('href', "https://oladitisodiq.github.io/")
      } else {
        element.setAttribute('href', "https://github.com/oladitisodiq/")
      }
    }
  }
}